...
estimators.append(NuSVR())


for est in estimators:
    print("estimator: ", est.__class__.__name__)
    print("params: ", est.get_params())
    est.fit(df_merc.drop(columns = 'price'), df_merc.price)
    df_merc['predicted'] = est.predict(df_merc.drop(columns = 'price'))
    df_merc['eval'] = (df_merc['predicted'] - df_merc['price'])**2
